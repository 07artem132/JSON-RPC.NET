...
- **`aot-readiness` (2.4.0):** made the source-gen discovery path **provably Native-AOT-clean** — `[DynamicallyAccessedMembers(PublicConstructors)]` on `AddJsonRpcCore<…>`'s 5 service generics (clears IL2091) + honest `[UnconditionalSuppressMessage]` at the reflection-fallback boundary (IL2026/IL3050/IL3000). Verified: **0 IL warnings** under `-p:IsAotCompatible=true`, and a real `dotnet publish -p:PublishAot=true` native binary (`aot-smoke/`) runs catalog-based discovery with no reflection (exit 0). Suite stays 125. ⚠ Library still does **not** set `<IsAotCompatible>true</IsAotCompatible>`: RPC *dispatch* still uses StreamJsonRpc's reflective `AddLocalRpcTarget`. A spike rejected replacing StreamJsonRpc and identified the in-library AOT path (`AddLocalRpcMethod(name, delegate)` — no AOT attributes); migrating dispatch to it is a behavior-changing follow-up (`aot-rpc-dispatch`), pending sign-off.
- **`aot-rpc-dispatch` (2.5.0):** made RPC **dispatch** Native-AOT-clean — the generator also emits an `IRpcMethodBinder` (separate opt-in `AddGeneratedRpcMethodBinder()`) that registers each RPC-interface method via source-generated `JsonRpc.AddLocalRpcMethod(name, delegate)` (no AOT attributes) instead of the reflective `AddLocalRpcTarget`. `RegisterServices` prefers the binder, else the annotated reflection fallback. Honors `[JsonRpcMethod]`/`[JsonRpcIgnore]` + camelCase; unsupported method shapes → `WSRPC002`. `aot-smoke` native binary binds dispatch via the generator (exit 0, no reflection). 3 new guard tests; suite 125 → 128. ⚠ Trade-off: binder exposes only interface methods, drops target events / RpcMarshalable / JsonRpcTargetOptions (reflection path kept). Does **not** flip `IsAotCompatible`: StreamJsonRpc 2.21.69 payload/envelope serialization isn't AOT-clean (IL3053 on StreamJsonRpc.dll) — the remaining limit is **upstream**.
- **Still open:** all 20 audit findings shipped/resolved; the AOT track (discovery + dispatch) is complete for the part we own. Remaining AOT limit is upstream (StreamJsonRpc formatter not AOT-clean) → `IsAotCompatible` stays off.
